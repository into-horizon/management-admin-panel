...
const OrderModel = ({ data, loading, type }: PropTypes) => {
  const [itemAction, setItemAction] = useState<string>('')
  const [itemId, setItemId] = useState('')
  const dispatch = useDispatch()
  const { isUpdating } = useSelector((state: RootState) => state.orders)
  const closeModel = () => {
    setItemAction('')
    setItemId('')
  }
  ...
  const updateItem = (e: FormEvent<HTMLFormElement>, item: OrderItemType) => {
    ...
    dispatch(
      updateOrderItemHandler({
        ...
        cancellation_reason: target.cancellation_reason?.value,
      }),
    )
    closeModel()
  }
            ...
            <CTable>
              <CTableHead>
                <CTableRow>
                  ...
                  <CTableHeaderCell>place order date</CTableHeaderCell>
                  <CTableHeaderCell>delivery date order date</CTableHeaderCell>
              ...
              <CTableBody>
                <CTableRow>
                  ...
                  <CTableDataCell>{order.grand_total}</CTableDataCell>
              ...
              <CTable align='middle'>
                <CTableHead>
                  <CTableRow>
                    ...
                    <CTableHeaderCell>status</CTableHeaderCell>
                    {type === 'pending' && <CTableHeaderCell>action</CTableHeaderCell>}
                  </CTableRow>
                </CTableHead>
                <CTableBody>
                  {Children.toArray(
                    order.items.map((item) => (
                      <CTableRow>
                        ...
                        <CTableDataCell>{item.store_name}</CTableDataCell>
                        ...
                        {item.status === 'pending' && (
                          <CTableDataCell>
                            ...
                            <CButton color='secondary' onClick={() => setItemId(item.id)}>
                              {isUpdating ? <CSpinner color='light' size='sm' /> : 'action'}
                            </CButton>
                          </CTableDataCell>
                        )}
                      </CTableRow>
