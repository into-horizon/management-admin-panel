...
import { addAccountHandler, updateAccountHandler } from '../../../store/bankAccount'
...
export const AccountModal = ({
  ...
  const submitHandler = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    let obj: { type: string; title: string; reference: string; id: string } = {
      ...
      reference: target.referenceNumber.value,
      id: '',
    }
    Promise.all([
      update && account
  ...
  return (
    <CModal onClose={onClose} visible={add || update} alignment='center'>
      <CModalHeader>
        ...
        <CRow xs={{ gutterY: 3 }} className='mrgn50 justify-content-md-center'>
          <CCol xs={8}>
            <CFormInput
              ...
              placeholder={t('title')}
              required
              defaultValue={account ? account.title : ''}
            />
          </CCol>
          <CCol xs={8}>
          ...
          <CCol xs={8}>
            <CFormInput
              ...
              defaultValue={account ? account.reference : ''}
              required
            />
          </CCol>
        </CRow>
