...
import { ProductType, QuantityDetailsType } from '../../../types'
// import { ProductType, QuantityDetailsType } from '../types'
...
const SizeAndColorModal = ({ product, updateSizeAndQuantity }: PropTypes) => {
  ...
  const productColors =
    _sizeAndColor?.map((val) => val.color).filter((val, i, a) => i === a.indexOf(val)) ?? []
  const productSizes =
    _sizeAndColor?.map((val) => val.size).filter((val, i, a) => i === a.indexOf(val)) ?? []
                  ...
                  {sizeAndColor.length > 0 &&
                    Children.toArray(
                      sizeAndColor.map((item) => (
                        <li className='m-5rem'>
                          <button type='button' onClick={() => removeSize(item.id)}>
                            X
                          </button>
