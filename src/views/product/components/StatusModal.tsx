...
const StatusModal = ({ product, updateProductStatus, callback: cb }: PropTypes) => {
  const [visible, setVisible] = useState(false)
  const [status, setStatus] = useState(product.status)
  const submitHandler = async (e: FormEvent<HTMLFormElement> & { target: { rejection_reason?: HTMLInputElement, status: HTMLSelectElement } }) => {
    e.preventDefault()
    ...
    await updateProductStatus(data, 'overview')
    setVisible(false)
    cb?.()
  }
  return (
