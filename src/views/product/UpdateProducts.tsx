...
const UpdateProduct = () => {
  ...
  const [secondCategory, setSecondCategory] = useState({
    visible: true,
    data: childCategories.filter((c) => c.parent_id === searched.parent_category_id),
  })
  const [thirdCategory, setThirdCategory] = useState({
    ...
    add: false,
    select: true,
    data: grandChildCategories.filter((g) => g.parent_id === searched.child_category_id),
  })
  ...
  useEffect(() => {
    ...
    setThirdCategory((x) => {
      return {
        ...x,
        data: grandChildCategories.filter((g) => g.parent_id === searched.child_category_id),
        selected: !!searched.grandchild_category_id,
      }
    })
  }, [searched.id])
  ...
  const submitHandler = (e: FormEvent<HTMLFormElement>, id: string) => {
    ...
    let obj = {
      ...
      grandchild_category_id: target.grandChildCategory
        ? target.grandChildCategory.value === 'default'
          ? null
    ...
    showOptionDialog({
      ...
      onConfirm: () => {
        dispatch(updateProductHandler({ ...searched, ...obj }))
        navigate(-1)
      },
    })
  }
  const categoryVisibility = (e: ChangeEvent<HTMLSelectElement>) => {
    ...
    setSecondCategory({
      data: childCategories.filter((c) => c.parent_id === e.target.value),
      visible: true,
    })
    setThirdCategory((x) => {
      return { ...x, data: [] }
  ...
  const categoryVisibility2 = (e: ChangeEvent<HTMLSelectElement>) => {
    setThirdCategory((x) => {
      return {
        ...x,
        data: grandChildCategories.filter((c) => c.parent_id === e.target.value),
        visible: true,
      }
    })
  }
  ...
  useEffect(() => {
    Promise.all([
      ...
      id && dispatch(getSearchedProductHandler(id)),
  ...
  useEffect(() => {
    setThirdCategory((x) => {
      return { ...x, select: !!searched.grandchild_category_id }
    })
  }, [searched.id])
  return (
    <>
      ...
      {searched?.id && (
        <CForm onSubmit={(e) => submitHandler(e, searched.id)}>
          {loading ? (
            <CSpinner />
          ) : (
            <CRow xs={{ gutterY: 5 }}>
