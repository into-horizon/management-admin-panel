...
type PropTypes = {
    createDiscountCode: (payload: DiscountCodeType) => Promise<void>
    callback: GetFunctionType
    params: ParamsType
}
const CreateCodeModal = ({ createDiscountCode, callback, params }: PropTypes) => {
    const [visible, setVisible] = useState(false)
    const [value, setValue] = useState<number>(0)
    const months = ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12']
    const submitHandler = (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        const _params: string[] = ['discount_code', 'expiry_date', 'min_order_amount', 'max_counter', 'discount', 'max_discount', 'number_of_time']
        type InputTypes = {
            ...
            <CModal visible={visible} alignment='center' onClose={() => setVisible(false)}>
                ...
                <CForm onSubmit={submitHandler} onReset={onClose}>
                    <CRow className='justify-content-center align-content-center' xs={{ gutter: 3 }}>
                        ...
                        <CCol xs={10}>
                            <CFormInput type='number' id='discount' step={0.01} placeholder='discount amount or percentage' min={0} value={value} onChange={e => setValue(parseInt(e.target.value))} />
                        </CCol>
                        <CCol xs={10}>
