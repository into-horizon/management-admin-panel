import React, { useState, Children, HTMLInputTypeAttribute, ChangeEvent } from "react";
...
type PropTypes ={
  data: {[key: string]: any}
  action?: (d:{[key: string]: any}) => Promise<void>
  field: string
  type?: HTMLInputTypeAttribute
  options? : Array<{ name:string , value:any }>
}

const EditableCell = ({ data, action, field, type = 'text', options = [] }:PropTypes) => {
  const [edit, setEdit] = useState(false);
  const [cell, setCell] = useState(data[field] ?? '');
  const onChange = (e : ChangeEvent<HTMLInputElement | HTMLSelectElement>) => setCell(e.target.value);
  const submit = () => {
    let _data = { ...data };
    _data[field] = cell;
    action?.(_data);
