...
import { NotificationType, ParamsType } from "src/types";
...
export const getNotifications = createAsyncThunk(
  ...
  async (payload: ParamsType, { dispatch, rejectWithValue }) => {
    try {
      const { data, count, types } = await Notification.getNotifications(
        payload
      );
  ...
  async (_, { dispatch, getState, rejectWithValue }) => {
    try {
      ...
      const { count } = await Notification.updateNotificationsAsSeen({
        id: state.notifications.data.map(
  ...
  extraReducers: (builder) => {
    builder.addCase(getNotifications.fulfilled, (state, action) => {
      state.data = action.payload?.data
        ? action.payload?.data.concat(state.data)
        : action.payload;
