...
export const getPendingStores = (payload: ParamsType) => async (dispatch: AppDispatch) => {
  try {
    ...
    if (e instanceof Error) {
      dispatch(updateToast({ status: 403, message: e.message, type: DialogResponseTypes.ERROR }))
    }
  }
}

export const updateStoreStatusHandler =
  (payload: StoreType, type: 'pending' | 'overview') =>
  async (dispatch: AppDispatch, state: () => RootState) => {
    try {
      ...
      if (status === 200) {
        ...
        dispatch(updateToast({ status: 200, message: message, type: DialogResponseTypes.UPDATE }))
        dispatch(addData({ ...x }))
      } else {
        dispatch(
...
export const updateStoreHandler =
  (payload: StoreType) => async (dispatch: AppDispatch, state: () => RootState) => {
    try {
...
export const updateStoreNameHandler =
  (payload: StoreType) => async (dispatch: AppDispatch, state: () => RootState) => {
    try {
...
export const populateStore =
  (payload: { id: string; title: string }) => (dispatch: AppDispatch) => {
    payload.id
      ? save('populated-store', payload, { path: '/' })
