...
const initialState : {store: {title?:string, id?:string} , duration: string}  = {store:load('populated-store')?  load('populated-store'): {}, duration: load('duration')??setDefaultDate() }
...
export const populateStore = (payload ?: typeof initialState) => (dispatch :  AppDispatch) =>{
    payload?.store?.id ? save('populated-store', payload.store,{path:'/'}) : remove('populated-store', {path:'/'})
    payload?.duration ? save('duration', payload.duration,{path:'/'}) :  remove('duration', {path:'/'})
