...
export const getPendingProducts =
  (payload: ParamsType) => async (dispatch: AppDispatch) => {
    try {
      let { data, message, status } = await Product.getProducts({
        ...payload,
        status: "pending",
      });
      if (status === 200) {
...
export const updateProductStatus =
  (
    ...
    type: "pending" | "overview" = "pending"
  ) =>
  async (dispatch: AppDispatch, state: () => RootState) => {
    try {
      let { status, data, message } = await Product.updateProductStatus(
        payload
      );
      if (status === 200) {
        ...
        dispatch(
          addData({
            [type]: {
              data: newState,
              count: type === "pending" ? count - 1 : count,
            },
          })
        );
        dispatch(
...
export const deleteProductPictureHandler =
  (payload: { picture_id: string }) =>
  async (dispatch: AppDispatch, state: () => RootState) => {
    try {
      let { status, message, data } = await Product.deleteProductPicture(
        payload
      );
      if (status === 200) {
        let { count, data: overview } = state().products.overview;
  ...
  (payload: any) => async (dispatch: AppDispatch) => {
    try {
      let {
        ...
        data: product,
        message,
      } = await Product.getProduct(payload);
  ...
  async (dispatch: AppDispatch, state: () => RootState) => {
    try {
      let {
        ...
        result: data,
        status,
      } = await Product.updateProduct(payload);
      if (status === 200) {
